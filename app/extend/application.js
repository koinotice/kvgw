...
module.exports = {
    ...
    error(status, msg) {
        const error = new Error(msg || "tokenid不能为空");
        error.status = status || 1000;
        return error;
        ...
        this.nats.subscribe(topic, async (data, reply, subject) => {
            switch (subject) {
                ...
                case "vgw.token.create":
                    ...
                    ctx.service.token.insert(data)
                    break
                case "vgw.order.withdraw":

                    ctx.service.order.insertOrderWithdraw(data)
                    break
