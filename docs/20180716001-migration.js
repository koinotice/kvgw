...
module.exports = {
    up: co.wrap(function* (db, Sequelize) {
        ...
        // 收藏表
        yield db.createTable('collections', {
        ...
        // 管理员表
        yield db.createTable('admin_users', {
    ...
    down: co.wrap(function* (db) {
        yield db.dropTable('users');//用户表
        yield db.dropTable('collections');//收藏表
        yield db.dropTable('clocks');//打卡记录表
        yield db.dropTable('shops');//店铺表
        yield db.dropTable('shop_detaileds');//店铺详情表
        yield db.dropTable('coupons');//优惠券表
        yield db.dropTable('articles');//文章表

        yield db.dropTable('admin_users');//管理员表
    }),
};
